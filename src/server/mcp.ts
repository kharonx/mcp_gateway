...
- OPTIONAL Salesforce toolsets (present only when the gateway has a Salesforce Connected App configured), all running through the user's OWN linked Salesforce login (salesforce-connection-status tells whether it is linked and where to link it) - never more than that Salesforce user can see or do: "salesforce" = SOQL/SOSL, object describe, records, account overview, reports; "salesforce-write" (if enabled) = create/update record, task, event, Chatter post, note - confirm=true after explicit approval, records appear under the user's name; "salesforce-delete" (only if the admin enabled it explicitly) = delete one record to the Recycle Bin - read the record first, name it to the user, confirm=true per record.`;
